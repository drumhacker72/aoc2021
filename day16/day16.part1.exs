defmodule Day16 do
    def literal(bits) do
        <<more::1, val::4, rest::bits>> = bits
        if more === 0 do
            {<<val::4>>, rest}
        else
            {restLit, restExtra} = literal(rest)
            {<<val::4, restLit::bits>>, restExtra}
        end
    end

    def packetsUntilSize(bits, size) do
        if size === 0 do
            {[], bits}
        else
            {p, rest} = packet(bits)
            {ps, rest2} = packetsUntilSize(rest, size - (bit_size(bits) - bit_size(rest)))
            {[p | ps], rest2}
        end
    end

    def packetsUntilCount(bits, count) do
        if count === 0 do
            {[], bits}
        else
            {p, rest} = packet(bits)
            {ps, rest2} = packetsUntilCount(rest, count - 1)
            {[p | ps], rest2}
        end
    end

    def packet(bits) do
        <<version::3, typeId::3, rest::bits>> = bits
        case typeId do
            4 ->
                {lit, rest2} = Day16.literal(rest)
                s = bit_size(lit)
                <<lit2::size(s)>> = lit
                {{version, lit2}, rest2}
            _ ->
                <<lengthTypeId::1, rest2::bits>> = rest
                case lengthTypeId do
                    0 ->
                        <<totalSize::15, rest3::bits>> = rest2
                        {ps, rest4} = packetsUntilSize(rest3, totalSize)
                        {{version, ps}, rest4}
                    1 ->
                        <<numSubpackets::11, rest3::bits>> = rest2
                        {ps, rest4} = packetsUntilCount(rest3, numSubpackets)
                        {{version, ps}, rest4}
                end
        end
    end

    def versions(p) do
        {version, content} = p
        if is_list(content) do
            version + Enum.sum(Enum.map(content, fn p2 -> versions(p2) end))
        else
            version
        end
    end
end

{:ok, a} = Base.decode16("")
{p, _} = Day16.packet(a)
IO.puts Day16.versions(p)
